...
const SelectJenisHukuman = (props) => {
  ...
  useEffect(() => {
    if (props?.value && props?.value?.value && props?.value?.value != '') {
      ApiCall.get('/master-jenis-hukuman/' + props?.value?.value + '/detail')
        .then((res) => {
          if (res?.data) {
            setSelectedOption({
              value: res?.data?.id,
  ...
  useEffect(() => {
    setIsLoading(true);
    ApiCall.post('/master-jenis-hukuman')
      .then((res) => {
...
export default SelectJenisHukuman;
